...
static void _rl_init_file_error (const char *, ...)  __attribute__((__format__ (printf, 1,
        2)));
#else
static void _rl_init_file_error ();
#endif

static rl_command_func_t *_rl_function_of_keyseq_internal PARAMS((const char *, size_t,
        Keymap, int *));
...
int
rl_bind_keyseq_if_unbound_in_map (const char *keyseq, rl_command_func_t *default_func,
                                  Keymap kmap) {
    rl_command_func_t *func;
    char *keys;
    int keys_len;
...
rl_generic_bind (int type, const char *keyseq, char *data, Keymap map) {
    ...
    for (i = 0; i < keys_len; i++) {
        ...
        if ((i + 1) < keys_len) {
            ...
            if (k.function && ((k.type == ISFUNC && k.function != rl_do_lowercase_version) ||
                               k.type == ISMACR)) {
                map[ANYOTHERKEY] = k;
                k.function = 0;
            }
