...
expand_prompt (char *pmt, int flags, int *lp, int *lip, int *niflp, int *vlp) {
    ...
    newlines_guess = (_rl_screenwidth > 0) ? APPROX_DIV(l,  _rl_screenwidth) : APPROX_DIV(l,
                     80);
    local_prompt_newlines = (int *) xrealloc (local_prompt_newlines,
                            sizeof (int) * (newlines_guess + 1));
...
init_line_structures (int minsize) {
    ...
    if (vis_lbreaks == 0) {
        ...
        line_state_visible->wrapped_line = (int *)xmalloc (line_state_visible->wbsize * sizeof (
                                               int));

        line_state_invisible->wbsize = inv_lbsize;
        line_state_invisible->wrapped_line = (int *)xmalloc (line_state_invisible->wbsize *
                                             sizeof (int));
#endif
...
#define CHECK_LPOS() \
    ...
    if (lpos >= _rl_screenwidth) {
        ...
        if (mb_cur_max > 1 && rl_byte_oriented == 0 && prompt_multibyte_chars > 0) {
            lpos = _rl_col_width (local_prompt, temp, local_prompt_len,
                                  1) - (wrap_offset - prompt_invis_chars_first_line);
        } else {
            ...
            for (linenum = 0; linenum <= inv_botlin; linenum++) {
                ...
                if (linenum == 0 && (mb_cur_max > 1 && rl_byte_oriented == 0) && OLD_CPOS_IN_PROMPT()) {
                    _rl_last_c_pos -= prompt_invis_chars_first_line;    /* XXX - was wrap_offset */
                           ...
                           _rl_last_c_pos != o_cpos &&
                           _rl_last_c_pos > (prompt_last_invisible - _rl_screenwidth -
...
static void
update_line (char *old, char *old_face, char *new, char *new_face, int current_line,
             int omax, int nmax, int inv_botlin) {
    ...
    if (_rl_quick_redisplay) {
        ...
        } else if ((nmax - W_OFFSET(current_line, wrap_offset)) < (omax - W_OFFSET (current_line,
                   visible_wrap_offset))) {
            goto clear_rest_of_line;
        } else {
            return;
         ...
         around on auto-wrapping terminals. */
            ...
            if (*ols && ((_rl_horizontal_scroll_mode &&
                ...
                if ((mb_cur_max > 1 && rl_byte_oriented == 0) && current_line == 0 &&
                        displaying_prompt_first_line && wrap_offset && ((nfd - new) <= prompt_last_invisible)) {
                    ADJUST_CPOS (wrap_offset);    /* XXX - prompt_invis_chars_first_line? */
                }
                return;
            }
