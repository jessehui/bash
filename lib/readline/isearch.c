...
opcode_dispatch:
    ...
    switch (cxt->lastc) {
        ...
        case -3:	/* C-H, DEL */
            ...
            if (cxt->search_string_index == 0) {
                ...
                wstart = _rl_find_prev_mbchar (cxt->search_string, cxt->search_string_index,
                                               MB_FIND_NONZERO);
                if (wstart >= 0) {
    ...
    for (cxt->sflags &= ~(SF_FOUND | SF_FAILED);; ) {
        ...
        while ((cxt->sflags & SF_REVERSE) ? (cxt->sline_index >= 0) : (cxt->sline_index <
                limit)) {
        ...
        do {
            ...
            if ((cxt->sflags & SF_REVERSE) ? (cxt->history_pos < 0) : (cxt->history_pos ==
                    cxt->hlen)) {
                cxt->sflags |= SF_FAILED;
                break;
            }
        ...
        cxt->sline_index = (cxt->sflags & SF_REVERSE) ? cxt->sline_len -
