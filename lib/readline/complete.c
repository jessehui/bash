...
static char **gen_completion_matches PARAMS((char *, int, int, rl_compentry_func_t *, int,
        int));
...
static char **
gen_completion_matches (char *text, int start, int end, rl_compentry_func_t *our_func,
...
rl_filename_completion_function (const char *text, int state) {
    ...
    if (state == 0) {
        ...
        if (rl_completion_found_quote && rl_filename_dequoting_function) {
            users_dirname = (*rl_filename_dequoting_function) (dirname,
