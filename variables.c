...
#if defined (ARRAY_VARS)
static SHELL_VAR *init_dynamic_array_var PARAMS((char *, sh_var_value_func_t *,
        sh_var_assign_func_t *, int));
static SHELL_VAR *init_dynamic_assoc_var PARAMS((char *, sh_var_value_func_t *,
        sh_var_assign_func_t *, int));
#endif
...
static SHELL_VAR *assign_comp_wordbreaks PARAMS((SHELL_VAR *, char *, arrayind_t,
        char *));
#endif
...
static SHELL_VAR *bind_variable_internal PARAMS((const char *, char *, HASH_TABLE *, int,
        int));
...
static SHELL_VAR *find_variable_nameref_context PARAMS((SHELL_VAR *, VAR_CONTEXT *,
        VAR_CONTEXT **));
static SHELL_VAR *find_variable_last_nameref_context PARAMS((SHELL_VAR *, VAR_CONTEXT *,
        VAR_CONTEXT **));
...
init_seconds_var () {
    ...
    INIT_DYNAMIC_VAR ("SECONDS", (v ? value_cell (v) : (char *)NULL), get_seconds,
                      assign_seconds);
    return v;
}
...
int flags;
{
    ...
    else if ((flags & ASS_NOEVAL) == 0 && (capcase_p (var) || uppercase_p (var) ||
                                           lowercase_p (var))) {
        if (flags & ASS_APPEND) {
...
int hflags, aflags;
{
    ...
    if (entry && invisible_p (entry) && nameref_p (entry)) {
        ...
        if (valid_array_reference (newval, 0)) {
            ...
            entry = assign_array_element (newval, make_variable_value (entry, value, aflags),
                                          aflags | ASS_NAMEREF);
            if (entry == 0) {
                return entry;
            }
...
int n;
{
    ...
    if (n > vlist->list_size) {
        vlist->list_size = n;
        vlist->list = (SHELL_VAR **)xrealloc (vlist->list,
                                              (vlist->list_size + 1) * sizeof (SHELL_VAR *));
    }
    return vlist;
}
...
int flags;
{
    ...
    if (var_hash_table == 0 || (HASH_ENTRIES (var_hash_table) == 0) || (vlist == 0 &&
            func == 0)) {
        return;
    }
...
PTR_T data;
{
    ...
    if (binding_table == 0) {
        if (shell_variables == global_variables)
            /* shouldn't happen */
        {
            binding_table = shell_variables->table = global_variables->table = hash_create (
...
int is_special;
{
    ...
    if (vc_istempscope (vcxt) == 0) {
        internal_error (
            _("pop_scope: head of shell_variables not a temporary environment scope"));
        return;
    }
