...
// TODO?: This may not be sufficient if there are nested script file?
...
char *script_name;
{
    ...
    free (filename);
    // printf("shell script dir path = %s\n", shell_script_dir_path);
...
int on_or_off;
{
    if (shopt_ind >= shopt_len) {
        shopt_len += 8;
        shopt_alist = (STRING_INT_ALIST *)xrealloc (shopt_alist,
                      shopt_len * sizeof (shopt_alist[0]));
    }
...
run_shopt_alist () {
    ...
    for (i = 0; i < shopt_ind; i++)
        if (shopt_setopt (shopt_alist[i].word,
                          (shopt_alist[i].token == '-')) != EXECUTION_SUCCESS) {
            exit (EX_BADUSAGE);
        }
    free (shopt_alist);
    shopt_alist = 0;
