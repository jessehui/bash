...
reset_completer_word_break_chars () {
    rl_completer_word_break_characters = perform_hostname_completion ? savestring (
            bash_completer_word_break_characters) : savestring (
...
char **dirname;
{
    ...
    if (no_symbolic_links == 0 && (local_dirname[0] != '.' || local_dirname[1])) {
        ...
        if (dircomplete_expand_relpath || ((local_dirname[0] != '/' && local_dirname[0] != '.') &&
                                           STREQ (temp1, temp2) == 0)) {
...
int key;
{
    ...
    cmd = (char *)rl_function_of_keyseq_len (rl_executing_keyseq, rl_key_sequence_length,
            cmd_xmap, &type);
