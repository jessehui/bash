...
bgp_resize () {
    ...
    if (bgpids.nalloc < nsize_cur && bgpids.nalloc < nsize_max) {
        bgpids.storage = (struct pidstat *)xrealloc (bgpids.storage,
                         nsize * sizeof (struct pidstat));
...
#if 0
    itrace ("realloc_jobs_list: reset js.j_current (%d) and js.j_previous (%d)", js.j_current,
            js.j_previous);
#endif
...
int job_index, dflags;
{
    ...
    if (temp->state == JDEAD) {
        ...
        if (js.c_reaped < 0) {
#ifdef DEBUG
            itrace("delete_job (%d pgrp %d): js.c_reaped (%d) < 0 ndel = %d js.j_ndead = %d",
...
#  ifdef DEBUG
        if (j == NO_JOB) {
            internal_warning ("add_process: process %5ld (%s) in the_pipeline", (long)p->pid,
                              p->command);
        }
#  endif
        if (PALIVE (p)) {
...
#if defined (PROCESS_SUBSTITUTION)
    if (procsubs.nproc > 0 && (p = procsub_search (pid)) && ((alive_only == 0 &&
            PRECYCLED(p) == 0) || PALIVE(p))) {
        return (p);
    }
#endif
...
#if defined (DEBUG)
            if (i < js.j_firstj && jobs[i]) {
                itrace("wait_for_background_pids: job %d non-null before js.j_firstj (%d)", i,
                       js.j_firstj);
            }
            if (i > js.j_lastj && jobs[i]) {
...
int flags;
{
...
#if defined (DEBUG)
            internal_warning ("wait_for: recursively setting old_sigint_handler to wait_sigint_handler: running_trap = %d",
                              running_trap);
#endif
        } else {
    ...
    /* XXX */
    if ((job != NO_JOB && JOBSTATE (job) == JSTOPPED) || (child &&
...
#if 0
        if (job == NO_JOB) {
            itrace("wait_for: job == NO_JOB, giving the terminal to shell_pgrp (%ld)",
                   (long)shell_pgrp);
        }
#endif
    ...
    if (job != NO_JOB) {
        if (interactive_shell && subshell_environment == 0) {
            ...
            if (job_control && IS_JOBCONTROL (job) && IS_FOREGROUND (job) &&
                ...
                if (signal_is_trapped (SIGINT) == 0 && (loop_level || (shell_compatibility_level > 32 &&
                                                        executing_list))) {
                    ADDINTERRUPT;
                }
...
#if 0
        itrace("waitchld: waitpid returns %d block = %d children_exited = %d", pid, block,
               children_exited);
#endif
        ...
        if (jobs[job] && IS_NOTIFIED (job) == 0) {
            ...
            if ((job_control == 0 && interactive_shell) ||
                ...
                hang onto the job corresponding to the last asynchronous
                 ...
                 a `wait'. */
                if (DEADJOB (job) && (interactive_shell ||
...
#if defined (DEBUG)
        internal_warning ("%d: maybe_give_terminal_to: terminal pgrp == %d shell pgrp = %d new pgrp = %d in_background = %d",
...
int force;
{
    ...
    if (force) {
        /* XXX could use js.j_firstj here */
        for (i = 0; i < js.j_jobslots; i++) {
            if (jobs[i] && DEADJOB (i) && (interactive_shell ||
...
#if defined (DEBUG)
        if (i < js.j_firstj && jobs[i]) {
            itrace("mark_dead_jobs_as_notified: job %d non-null before js.j_firstj (%d)", i,
                   js.j_firstj);
        }
        if (i > js.j_lastj && jobs[i]) {
            itrace("mark_dead_jobs_as_notified: job %d non-null after js.j_lastj (%d)", i,
                   js.j_lastj);
        }
#endif
        if (jobs[i] && DEADJOB (i)) {
...
# if 0
    if (ndeadproc != js.c_reaped) {
        itrace("mark_dead_jobs_as_notified: ndeadproc (%d) != js.c_reaped (%d)", ndeadproc,
               js.c_reaped);
    }
# endif
    if (ndead != js.j_ndead) {
...
#if 0
    itrace("mark_dead_jobs_as_notified: child_max = %d ndead = %d ndeadproc = %d",
           js.c_childmax, ndead, ndeadproc);
#endif
...
static int children_routine_simple_cmd (struct nofork_child_args *recv_args) {
    ...
    int pipe_in = recv_args->pipe_in;
    int pipe_out = recv_args->pipe_out;
    // itrace("child thread [%ld] pipe_in = %d, pipe_out = %d", syscall(SYS_gettid), pipe_in, pipe_out);
...
pthread_t
make_child_without_fork_pipe_cmd(command, flags, pipe_in, pipe_out, cmd_list)
char *command;
int flags;
int pipe_in;
int pipe_out;
