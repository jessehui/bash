...
int interact;
{
    ...
    history_expansion_inhibited = (interact == 0) ? 1 - histexp_flag :
...
char *line;
{
    ...
    if (current_command_line_count > 1) {
        if (current_command_first_line_saved &&
                ((parser_state & PST_HEREDOC) || literal_history || dstack.delimiter_depth != 0 ||
                 is_comment != 1)) {
...
const char *line;
{
    ...
    hdrlen = snprintf (loghdr, sizeof(loghdr), "HISTORY: PID=%d UID=%d", getpid(),
                       current_user.uid);
    msglen = strlen (line);
...
history_number () {
    using_history ();
    return ((remember_on_history ||
             enable_history_list) ? history_base + where_history () : 1);
}
