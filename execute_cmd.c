...
static int execute_simple_command PARAMS((SIMPLE_COM *, int, int, int,
        struct fd_bitmap *));
static int execute_builtin PARAMS((sh_builtin_func_t *, WORD_LIST *, int, int));
static int execute_function PARAMS((SHELL_VAR *, WORD_LIST *, int, struct fd_bitmap *,
                                    int, int));
...
struct fd_bitmap *fds_to_close;
{
    ...
    if (command->type == cm_coproc) {
        return (last_command_exit_value = execute_coproc (command, pipe_in, pipe_out,
                                          fds_to_close));
    }
#endif
    ...
    if (command->type == cm_subshell ||
        ...
        p = savestring (tcmd);
        paren_pid = getpid();
        pthread_t child_thread = make_child_without_fork_for_subshell(p, fork_flags, pipe_in,
                                 pipe_out);
        // endif
...
//             /* run exit trap for : | ( ...; ) and ( ...; ) | : */
//             s += user_subshell == 0 && command->type == cm_group && (pipe_in != NO_PIPE ||
//                     pipe_out != NO_PIPE) && asynchronous == 0;

//             last_command_exit_value = execute_in_subshell (command, asynchronous, pipe_in, pipe_out,
//                                       fds_to_close);
//             if (s) {
...
struct fd_bitmap *fds_to_close;
{
    ...
    if ((user_subshell || user_coproc) && (tcom->type == cm_simple ||
    ...
    if (result == EXITPROG) {
        ...
        return_code = execute_command_internal ((COMMAND *)tcom, asynchronous, NO_PIPE, NO_PIPE,
                                                fds_to_close);
    }
...
#if 0
    itrace ("execute_coproc (%s): [%d] %s", command->value.Coproc->name, coproc_pid,
            the_printed_command);
#endif
...
struct fd_bitmap *fds_to_close;
{
    ...
    if (lastpipe_opt && job_control == 0 && asynchronous == 0 && pipe_out == NO_PIPE &&
            prev > 0) {
...
#if 0
    if (signal_in_progress (DEBUG_TRAP) == 0 && (this_command_name == 0 ||
...
struct fd_bitmap *fds_to_close;
{
    ...
    {
        FREE (the_printed_command_except_trap);
        the_printed_command_except_trap = the_printed_command ? savestring (
                                              the_printed_command) : (char *)0;
    }
...
int async, subshell;
{
    ...
    if (funcnest_max > 0 && funcnest >= funcnest_max) {
        internal_error (_("%s: maximum function nesting level exceeded (%d)"), var->name,
                        funcnest);
    ...
    if (return_trap && (signal_in_progress (DEBUG_TRAP) || ((trace_p (var) == 0) &&
...
char **args, * *env;
{
    ...
    if (i != ENOEXEC) {
        ...
        last_command_exit_value = (i == ENOENT) ?  EX_NOTFOUND :
