     ...
     struct fd_bitmap *fds_to_close;
{
  ...
  if (command->type == cm_subshell ||
	...
	/* Otherwise we defer setting line_number */
      ...
      p = savestring (tcmd);
      paren_pid = getpid();
      pthread_t child_thread = make_child_without_fork("subshell", p, fork_flags, pipe_in,
                                pipe_out, 0, NULL);
      // endif
	 ...
	 the process/job associated with this child. */
      fork_flags = async ? FORK_ASYNC : 0;
        ...
        pid_t child_pid = make_child_without_fork ("pipe_cmd", p, fork_flags, pipe_in, pipe_out, 0,
                        simple_command);
        // pid_t child_pid = make_child_without_fork_pipe_cmd(p, fork_flags, pipe_in, pipe_out,
        //                   simple_command);
        if (child_pid > 0) {
  ...
  if (nofork && pipe_in == NO_PIPE && pipe_out == NO_PIPE)
  ...
  else
    {
      ...
      pid = make_child_without_fork ("simple_cmd", p, fork_flags, pipe_in, pipe_out,
                      NULL, command_simple);
      // pid = make_child_without_fork_simple_cmd ( p, fork_flags, pipe_in, pipe_out,
      //                 command_simple);
