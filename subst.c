...
static char *extract_delimited_string PARAMS((char *, int *, char *, char *, char *,
        int));
...
#if defined (HANDLE_MULTIBYTE)
static int match_wpattern PARAMS((wchar_t *, char **, size_t, wchar_t *, int, char **,
                                  char **));
#endif
...
#endif
static char *parameter_brace_remove_pattern PARAMS((char *, char *, int, char *, int, int,
        int));
...
#endif
static char *parameter_brace_transform PARAMS((char *, char *, int, char *, int, int, int,
        int));
...
static WORD_DESC *parameter_brace_expand_word PARAMS((char *, int, int, int,
        arrayind_t *));
static char *parameter_brace_find_indir PARAMS((char *, int, int, int));
static WORD_DESC *parameter_brace_expand_indir PARAMS((char *, int, int, int, int *,
        int *));
static WORD_DESC *parameter_brace_expand_rhs PARAMS((char *, char *, int, int, int, int *,
        int *));
...
static int verify_substring_values PARAMS((SHELL_VAR *, char *, char *, int, intmax_t *,
        intmax_t *));
static int get_var_and_type PARAMS((char *, char *, arrayind_t, int, int, SHELL_VAR **,
                                    char **));
static char *mb_substring PARAMS((char *, int, int));
static char *parameter_brace_substring PARAMS((char *, char *, int, char *, int, int,
        int));
...
static char *parameter_brace_casemod PARAMS((char *, char *, int, int, char *, int, int,
        int));

static WORD_DESC *parameter_brace_expand PARAMS((char *, int *, int, int, int *, int *));
static WORD_DESC *param_expand PARAMS((char *, int *, int, int *, int *, int *, int *,
                                       int));
...
int start, open, close, flags;
{
    ...
    while (c = string[i]) {
        ...
        } else if ((flags & 1) == 0 && c == '$' && (string[i + 1] == LPAREN ||
                   string[i + 1] == LBRACE)) {
            si = i + 2;
            if (string[si] == '\0') {
                CQ_RETURN(si);
    ...
    while (c = string[i]) {
        ...
        /* special case for programmable completion which takes place before
           ...
           `regular' single-quoted strings. */
            ...
            i++;
            continue;
        } else if (c == '$' && ((skipcmd && string[i + 1] == LPAREN) ||
                                string[i + 1] == LBRACE)) {
            si = i + 2;
            if (string[si] == '\0') {
                CQ_RETURN(si);
...
int quoted, pflags;
{
    ...
    if (pchar == '*' && (quoted & Q_DOUBLE_QUOTES)) {
        ...
        ret = expand_no_split_dollar_star ? string_list_dollar_star (list, quoted,
                0) : string_list_dollar_at (list, quoted, 0);    /* Posix interp 888 */
           ...
           later splitting. */
    {
        ...
        ret = string_list ((quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) ? quote_list (
                               list) : list);
    }
...
int quoted;
{
    ...
    if (string[i]) {
        ...
        td.flags = W_NOPROCSUB |
                   W_NOTILDE;	/* don't want process substitution or tilde expansion */
#if 0	/* TAG: bash-5.2 */
...
char **sp, * *ep;
{
    ...
    if (pat[0] != '*' || (pat[0] == '*' && pat[1] == LPAREN && extended_glob) ||
        ...
        p1 = pat;
        if ((mtype != MATCH_BEG) && (*p1 != '*' || (*p1 == '*' && p1[1] == LPAREN &&
                                     extended_glob))) {
            *p++ = '*';
        }
        while (*p1) {
...
char **sp, * *ep;
{
    ...
    switch (mtype) {
        case MATCH_ANY:
            for (n = 0; n <= wstrlen; n++) {
                n2 = simple ? (WFOLD(*wpat) == WFOLD(wstring[n])) : match_pattern_wchar (wpat,
                        wstring + n, FNMATCH_IGNCASE);
                if (n2) {
...
int open_for_read_in_child;
{
    ...
    if (pid == 0) {
        ...
        setup_async_signals ();
#if 0
        if (open_for_read_in_child == 0 && old_interactive && (bash_input.type == st_stdin ||
    ...
    while (1) {
        ...
        from the rest of the word expansions (word splitting and globbing.)
         This is essentially quote_escapes inline. */
            ...
            istring[istring_index++] = CTLESC;
        } else if ((skip_ctlnul == 0 && c == CTLNUL) || (c == ' ' && (ifs_value &&
                   *ifs_value == 0))) {
            istring[istring_index++] = CTLESC;
        }
...
expand_arrayref:
        ...
        if (var_isset (var) && invisible_p (var) == 0) {
            ...
            if ((pflags & PF_ALLINDS) && assoc_p (var)) {
                tt = temp = assoc_empty (assoc_cell (var)) ? (char *)NULL : assoc_to_string (assoc_cell (
                                var), " ", quoted);
            } else if ((pflags & PF_ALLINDS) && array_p (var)) {
                tt = temp = array_empty (array_cell (var)) ? (char *)NULL : array_to_string (array_cell (
                                var), " ", quoted);
            } else if (assoc_p (var)) {
...
static WORD_DESC *
parameter_brace_expand_indir (name, var_is_special, quoted, pflags, quoted_dollar_atp,
                              contains_dollar_at)
char *name;
int var_is_special, quoted, pflags;
int *quoted_dollar_atp, *contains_dollar_at;
...
int op, quoted, pflags, *qdollaratp, *hasdollarat;
{
    ...
    if (l) {
        ...
        if (l->next == 0 && (quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) &&
                QUOTED_NULL (temp) && QUOTED_NULL (l->word->word) && (l->word->flags & W_HASQUOTEDNULL)) {
    ...
    w->word = (quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT)) ? quote_string (
                  t1) : quote_escapes (t1);
...
char **valp;
{
    ...
    *varp = (SHELL_VAR *)NULL;
    ...
    } else if ((v = find_variable (vname)) && (invisible_p (v) == 0) && (assoc_p (v) ||
               array_p (v))) {
        vtype = VT_ARRAYMEMBER;
        *varp = v;
        *valp = assoc_p (v) ? assoc_reference (assoc_cell (v),
                                               "0") : array_reference (array_cell (v), 0);
    } else
#endif
    {
...
char *s;
{
    ...
    val = (v && (invisible_p (v) ||
                 var_isset (v) == 0)) ? (char *)NULL : sh_quote_reusable (s, 0);
...
int itype, quoted, atype;
{
    ...
    if (val == 0 && (invisible_p (v) || var_isset (v) == 0))
        ...
        ret = (quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT)) ? quote_string (
                  val) : quote_escapes (val);
        free (val);
        val = ret;
    }
...
#  endif /* CASEMOD_TOGGLECASE */
    ...
    if (want_indir) {
        tdesc = parameter_brace_expand_indir (name + 1, var_is_special, quoted,
                                              pflags | local_pflags, quoted_dollar_atp, contains_dollar_at);
    ...
    if (c && c != RBRACE) {
        ...
        value = extract_dollar_brace_string (string, &sindex, quoted, (c == '%' || c == '#' ||
                                             c == '/' || c == '^' || c == ',' || c == ':') ? SX_POSIXEXP | SX_WORD : SX_WORD);
    ...
    *indexp = sindex;
    ...
    if (want_substring || want_patsub || want_casemod || c == '@' || c == '#' || c == '%' ||
            c == RBRACE) {
        if (var_is_set == 0 && unbound_vars_is_error && ((name[0] != '@' && name[0] != '*') ||
                name[1]) && all_element_arrayref == 0) {
            set_exit_status (EXECUTION_FAILURE);
            err_unboundvar (name);
            FREE (value);
            FREE (temp);
    ...
    if (want_substring) {
        temp1 = parameter_brace_substring (name, temp, ind, value, quoted, pflags,
        ...
        temp1 = parameter_brace_patsub (name, temp, ind, value, quoted, pflags,
                                        (tflag & W_ARRAYIND) ? AV_USEIND : 0);
        FREE (value);
        FREE (temp);
        ...
        temp1 = parameter_brace_casemod (name, temp, ind, modspec, value, quoted, pflags,
                                         (tflag & W_ARRAYIND) ? AV_USEIND : 0);
        FREE (value);
        FREE (temp);
...
bad_substitution:
            ...
            if (shell_compatibility_level <= 43) {
                ...
                return ((posixly_correct &&
                         interactive_shell == 0) ? &expand_wdesc_fatal : &expand_wdesc_error);
            }
        ...
        case '@':
            temp1 = parameter_brace_transform (name, temp, ind, value, c, quoted, pflags,
                                               (tflag & W_ARRAYIND) ? AV_USEIND : 0);
            free (temp);
            free (value);
        ...
        case '%':	/* ${param%[%]pattern} */
            ...
            temp1 = parameter_brace_remove_pattern (name, temp, ind, value, c, quoted,
                                                    (tflag & W_ARRAYIND) ? AV_USEIND : 0);
            free (temp);
            free (value);
    ...
    switch (c) {
        ...
        case '*':		/* `$*' */
            ...
            if ((quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) && list == 0) {
                ...
                temp = (quoted & (Q_DOUBLE_QUOTES | Q_PATQUOTE)) ? string_list_dollar_star (list, quoted,
                        0) : string_list (list);
                if (temp) {
                ...
                if (expand_no_split_dollar_star && quoted == 0 && ifs_is_set == 0 &&
                    ...
                    FREE (temp1);
                } else if (expand_no_split_dollar_star && quoted == 0 && ifs_is_set && ifs_is_null == 0 &&
                           (pflags & PF_ASSIGNRHS)) {
...
#if 0
                    internal_warning (
                        _("future versions of the shell will force evaluation as an arithmetic substitution"));
#endif
                    goto comsub;
                }
...
#if defined (ARRAY_VARS)
                if (temp && *temp && valid_array_reference (temp, 0)) {
                    tdesc = parameter_brace_expand_word (temp, SPECIAL_VAR (temp, 0), quoted, pflags,
                                                         (arrayind_t *)NULL);
                    if (tdesc == &expand_wdesc_error || tdesc == &expand_wdesc_fatal) {
                        return (tdesc);
                    }
...
WORD_LIST *tlist, *wcmd;
{
    ...
    if ((tlist->word->flags & (W_ASSIGNASSOC | W_ASSNGLOBAL)) == (W_ASSIGNASSOC |
            W_ASSNGLOBAL)) {
        opts[opti++] = 'g';
        opts[opti++] = 'A';
...
int eflags;
{
    ...
    if ((eflags & WEXP_VARASSIGN) && subst_assign_varlist) {
        ...
        is_builtin_or_func = (new_list && new_list->word &&
                              (find_shell_builtin (new_list->word->word) || find_function (new_list->word->word)));
        /* Posix says that special builtins exit if a variable assignment error
        occurs in an assignment preceding it. */
        is_special_builtin = (posixly_correct && new_list && new_list->word &&
