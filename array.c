...
static char *array_to_string_internal PARAMS((ARRAY_ELEMENT *, ARRAY_ELEMENT *, char *,
        int));
...
int	n, flags;
{
    ...
    INVALIDATE_LASTREF(a);
    for (i = 0, ret = ae = element_forw(a->head); ae != a->head &&
            i < n; ae = element_forw(ae), i++)
        ;
    if (ae == a->head) {
...
int	starsub, quoted, pflags;
{
     ...
     * index).
     */
    for (p = element_forw(p); p != array_head(a) &&
            start > element_index(p); p = element_forw(p))
        ;
