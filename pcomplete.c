...
static WORD_LIST *build_arg_list PARAMS((char *, const char *, const char *, WORD_LIST *,
                                        int));
...
const char *text;
{
    ...
    GEN_XCOMPS(flags, CA_COMMAND, text, command_word_completion_function, cmatches, ret,
               tmatches);
    GEN_XCOMPS(flags, CA_FILE, text, pcomp_filename_completion_function, cmatches, ret,
               tmatches);
    GEN_XCOMPS(flags, CA_USER, text, rl_username_completion_function, cmatches, ret,
               tmatches);
    GEN_XCOMPS(flags, CA_GROUP, text, bash_groupname_completion_function, cmatches, ret,
               tmatches);
...
#endif
    ret = split_at_delims (line, llen, delims, sentinel, SD_NOQUOTEDELIM | SD_COMPLETE, nwp,
                           cwp);
    return (ret);
}
...
#ifdef DEBUG
            if (progcomp_debug) {
                debug_printf ("gen_shell_function_matches (%p, %s, %s, %p, %d, %d) -->", cs, cmd, word,
                              lwords, nw, cw);
                strlist_print (tmatches, "\t");
                rl_on_new_line ();
            }
...
#ifdef DEBUG
            if (progcomp_debug) {
                debug_printf ("gen_command_matches (%p, %s, %s, %p, %d, %d) -->", cs, cmd, word, lwords,
                              nw, cw);
                strlist_print (tmatches, "\t");
                rl_on_new_line ();
            }
