...
#line 575 "/Users/chet/src/bash/src/parse.y"
            {
                source.filename = (yyvsp[-2].word);
                redir.filename = (yyvsp[0].word);
                (yyval.redirect) = make_redirection (source, r_deblank_reading_until, redir,
                                                     REDIR_VARASSIGN);
                push_heredoc ((yyval.redirect));
            }
#line 2228 "y.tab.c"
...
#line 630 "/Users/chet/src/bash/src/parse.y"
            {
                source.filename = (yyvsp[-2].word);
                redir.dest = (yyvsp[0].number);
                (yyval.redirect) = make_redirection (source, r_duplicating_output, redir,
                                                     REDIR_VARASSIGN);
            }
#line 2318 "y.tab.c"
            break;
...
#line 648 "/Users/chet/src/bash/src/parse.y"
            {
                source.filename = (yyvsp[-2].word);
                redir.filename = (yyvsp[0].word);
                (yyval.redirect) = make_redirection (source, r_duplicating_input_word, redir,
                                                     REDIR_VARASSIGN);
            }
#line 2348 "y.tab.c"
            break;
...
#line 666 "/Users/chet/src/bash/src/parse.y"
            {
                source.filename = (yyvsp[-2].word);
                redir.filename = (yyvsp[0].word);
                (yyval.redirect) = make_redirection (source, r_duplicating_output_word, redir,
                                                     REDIR_VARASSIGN);
            }
#line 2378 "y.tab.c"
            break;
...
#line 811 "/Users/chet/src/bash/src/parse.y"
            {
                (yyval.command) = make_for_command ((yyvsp[-5].word), add_string_to_list ("\"$@\"",
                                                    (WORD_LIST *)NULL), (yyvsp[-1].command), word_lineno[word_top]);
                if (word_top > 0) { word_top--; }
            }
#line 2646 "y.tab.c"
...
#line 821 "/Users/chet/src/bash/src/parse.y"
            {
                (yyval.command) = make_for_command ((yyvsp[-8].word), REVERSE_LIST ((yyvsp[-5].word_list),
                                                    WORD_LIST *), (yyvsp[-1].command), word_lineno[word_top]);
                if (word_top > 0) { word_top--; }
            }
#line 2664 "y.tab.c"
            break;
...
#line 831 "/Users/chet/src/bash/src/parse.y"
            {
                (yyval.command) = make_for_command ((yyvsp[-7].word), (WORD_LIST *)NULL,
...
#line 869 "/Users/chet/src/bash/src/parse.y"
            {
                (yyval.command) = make_select_command ((yyvsp[-4].word), add_string_to_list ("\"$@\"",
                                                       (WORD_LIST *)NULL), (yyvsp[-1].command), word_lineno[word_top]);
                if (word_top > 0) { word_top--; }
            }
#line 2740 "y.tab.c"
...
#line 879 "/Users/chet/src/bash/src/parse.y"
            {
                (yyval.command) = make_select_command ((yyvsp[-5].word), add_string_to_list ("\"$@\"",
                                                       (WORD_LIST *)NULL), (yyvsp[-1].command), word_lineno[word_top]);
                if (word_top > 0) { word_top--; }
            }
#line 2758 "y.tab.c"
...
#line 889 "/Users/chet/src/bash/src/parse.y"
            {
                (yyval.command) = make_select_command ((yyvsp[-8].word),
...
#line 1287 "/Users/chet/src/bash/src/parse.y"
            {
                ...
                tc = (yyvsp[-3].command)->type == cm_simple ? (COMMAND *)(
                         yyvsp[-3].command)->value.Simple : (yyvsp[-3].command);
                sd.dest = 2;
                rd.dest = 1;
        ...
        while (1) {
            ...
            if (shell_input_line_size > (SIZE_MAX - 256)) {
                ...
                if (n <= 2) {	/* we have to save 1 for the newline added below */
                    if (truncating == 0) {
                        internal_warning(
                            _("shell_getc: shell_input_line_size (%zu) exceeds SIZE_MAX (%lu): line truncated"),
...
time_command_acceptable () {
    ...
    if (posixly_correct && shell_compatibility_level > 41) {
        ...
        while (i < shell_input_line_len && (shell_input_line[i] == ' ' ||
                                            shell_input_line[i] == '\t')) {
            i++;
        }
...
char *tokstr;
{
    ...
    /* We allow a `do' after a for ((...)) without an intervening
       list_terminator */
    if (last_read_token == ARITH_FOR_EXPRS && tokstr[0] == 'd' && tokstr[1] == 'o' &&
            !tokstr[2]) {
        return (DO);
    }
...
int *lenp, flags;
{
    ...
    while (count) {
        ...
        we've already prepended CTLESC to single-quoted results of $'...'.
         ...
         reparse, too. */
        else if MBTEST((parser_state & PST_REPARSE) && open == '\'' && (ch == CTLESC ||
                       ch == CTLNUL)) {
            RESIZE_MALLOCED_BUFFER (ret, retind, 1, retsize, 64);
            ret[retind++] = ch;
            continue;
        ...
        if (open != close) {	/* a grouping construct */
            if MBTEST(shellquote (ch)) {
                ...
                if MBTEST((tflags & LEX_WASDOL) && ch == '\'' && (extended_quote ||
                          (rflags & P_DQUOTE) == 0)) {
                    /* Translate $'...' here. */
                    ...
                    if ((shell_compatibility_level > 42) && (rflags & P_DQUOTE) &&
                            (dolbrace_state == DOLBRACE_QUOTE2) && (flags & P_DOLBRACE)) {
                ...
                APPEND_NESTRET ();
                FREE (nestret);
            } else if ((flags & (P_ARRAYSUB | P_DOLBRACE)) && (tflags & LEX_WASDOL) && (ch == '(' ||
                       ch == '{' || ch == '[')) {	/* ) } ] */
...
eof_error:
        ...
        if (tflags & LEX_RESWDOK) {
            ...
            } else if MBTEST((tflags & LEX_CKCOMMENT) && ch == '#' && (lex_rwlen == 0 ||
        ...
        if MBTEST((tflags & LEX_INCOMMENT) == 0 && (tflags & LEX_CKCASE) && ch == '<') {
            ...
            if (peekc == ch) {
                ...
                continue;
                         ...
                         (((tflags & LEX_RESWDOK) && lex_rwlen == 0) || ((tflags & LEX_INWORD) &&
                                 lex_wlen == 0))) {
        ...
        } else if MBTEST(((flags & P_FIRSTCLOSE) == 0) && (tflags & LEX_INCASE) == 0 &&
        ...
        if MBTEST(shellquote (ch)) {
            ...
            APPEND_NESTRET ();
            FREE (nestret);
        } else if MBTEST((tflags & LEX_WASDOL) && (ch == '(' || ch == '{' ||
                         ch == '['))	/* ) } ] */
...
#if DEBUG
    if (base[*indp] != ')') {
        itrace("xparse_dolparen:%d: base[%d] != RPAREN (%d), base = `%s'", line_number, *indp,
               base[*indp], base);
    }
    if (*indp < orig_ind) {
        itrace("xparse_dolparen:%d: *indp (%d) < orig_ind (%d), orig_string = `%s'", line_number,
               *indp, orig_ind, ostring);
    }
#endif
    ...
    if (tok == COND_END) {
        ...
        if (tok == WORD) {
            ...
            if (etext = error_token_from_token (tok)) {
                parser_error (line_number, _("unexpected argument `%s' to conditional unary operator"),
                              etext);
                free (etext);
            } else {
        ...
        if (tok == WORD && test_binop (yylval.word->word)) {
            op = yylval.word;
            if (op->word[0] == '=' && (op->word[1] == '\0' || (op->word[1] == '=' &&
                                       op->word[2] == '\0'))) {
        ...
        if (tok == WORD) {
            ...
            if (etext = error_token_from_token (tok)) {
                parser_error (line_number, _("unexpected argument `%s' to conditional binary operator"),
                              etext);
                free (etext);
            } else {
...
char *message;
{
    ...
    if (message) {
        ...
        last_command_exit_value = (executing_builtin &&
    ...
    if (current_token != 0 && EOF_Reached == 0 &&
